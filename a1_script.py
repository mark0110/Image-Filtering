...
msg = 'Welcome to our Assignment 1!\n\nOur program will display a series of images and textboxs to answer the questions as outlined in the assignment document. Be advised that the execution time for some of the images is long. \n\nPress Okay to continue...'
popup.popupmsg(msg)

msg = "Instructions:\n\nThe images and textboxs may appear in the back of your current window. If an image is expected, please look around your screen to find the output.If the current output is an image, then the next output will not appear until you press any keyboard button (i.e space bar).If the current output is a textbox, then it is necessary to to press the okay button to proceed.\n\nPress okay to continue..."
popup.popupmsg(msg)
...
msg = "The next 3 image's show the outputs for questions 3 4 and 5 from the first section. The first image displayed is the convolved image using the code written by us (not using the built in command). The second image is the image convolved using the built in command. And finaly the third image will show the difference of the 2 images (or the lack of the difference).\n\nThe execution time is approximately 30 seconds.\n\nPress okay to continue..."
popup.popupmsg(msg)
